...
class MSA(nn.Module):
    def __init__(self, dim, heads=4, head_dim=16, dropout=0.0):
        super().__init__()
    ...
    def forward(self, x):
        ...
        return self.dropout2(self.msa_out(weighted_values))


class Transformer(nn.Module):
    def __init__(self, dim, depth, head_count, head_dim, mlp_dim, dropout):
        super().__init__()

        self.layers = nn.ModuleList([])
        for _ in range(depth):
            self.layers.append(nn.ModuleList([
                MSA(dim, head_count, head_dim, dropout),
...
class ViTinyDropout(nn.Module):
    def __init__(self, image_size, patch_size, num_classes, depth, dim, mlp_dim, head_count=4, head_dim=16,
        ...
        self.transformer = Transformer(dim, depth, head_count, head_dim, mlp_dim, dropout)
